...
const ls = {
  ...
  displayResults : function() {
    ...
    // create array with each word/word fragment of query as its own item in array
    const queryFragments = queryValue.split(' ');
    const totalFragments = queryFragments.length;
    ...
    const article = data.getElementsByTagName('article_title')[i].firstChild.nodeValue.toLowerCase();
      ...
      for (let j=0; j < totalFragments; j++) {
        ...
        // if current word/word fragment matches article, continue checking next word/word fragment in query
        if (article.match(queryFragments[j])){

            // only display article if all words/word fragments in query found match in article title (aka if loop made it to last word in array)
            if (j === (totalFragments - 1)) {
          ...
          // if current word/word fragment didn't match, skip to next article
          else {
            break;
          }
