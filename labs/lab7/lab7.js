const election = {
  ...
  checkData : function() {
    ...
    if (errorsArray.length) {
       ...
       // add to localStorage based on name and value of input field
       for (let i=0; i<election.allInputs; i++) {
         if (election.inputFields[i].type === 'button') { continue; } // skip button
         localStorage.setItem(election.inputFields[i].name, election.inputFields[i].value);
       }

       // add to localStorage based on name and value of selected option
       for (i=0; i<election.allSelects; i++) {
         localStorage.setItem(election.selectMenus[i].name, election.selectMenus[i].value);
       }
