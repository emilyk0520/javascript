...
const wf = {
    ...
    init : function() {
      ...
      // call JSON file to generate table
      this.fetchJSON('issues-data.json');

      // call JSON file to generate 'Recent Changes' sidebar
      this.fetchJSON('recent-changes.json');
    ...
    populateJSONData : function(data) {
      ...
      // based on if JSON data contains changes or issues, set up different HTML
      if (data.changes) {
      ...
      if (data.issues) {
        ...
        for (let i=0; i<totalIssues; i++) {
          ...
          str += '<tr data-priority="' + data.issues[i].priority.toLowerCase() + '">';
          str += '<th scope="row">' + data.issues[i].id + '</th>';
          str += '<td>' + data.issues[i].type + '</td>';
          str += '<td><a href="' + data.issues[i].project + '-' + data.issues[i].id + '.html">' + data.issues[i].desc + '</a></td>';
          str += '<td>' + data.issues[i].priority + '</td>';
          str += '<td>' + data.issues[i].assigned + '</td>';
          str += '</tr>';

        }

        // insert recent changes as li items into created ul
        wf.dataRowHolder.innerHTML = str;
