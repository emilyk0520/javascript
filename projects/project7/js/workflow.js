...
const wf = {
    ...
    init : function() {
      ...
      // create buttons to control table effects and recent changes ul
      this.createPageContent();

      // restore button and sort settings
      ...
      this.sendRequest('recent-changes.json?' + Math.random(), this.populateRecentChanges);

      // call JSON every 30 seconds
      const changesRefresh = setInterval(this.pullNewChanges, 30000);
    ...
    createPageContent : function() {
      ...
      wf.dataTable.insertAdjacentHTML('beforebegin', '<form id="actionForm" method="post" action="#"></form>');
      wf.buttonsArea = document.getElementById('actionForm');
      ...
      // creating ul for recent changes area
      wf.recentChangesList = document.createElement('ul');
      wf.recentChanges.appendChild(wf.recentChangesList);
    ...
    populateRecentChanges : function(xhr) {
      ...
      wf.recentChangesList.innerHTML = str;

    },

    // function called periodically to refresh the xml data
    pullNewChanges : function() {
      wf.sendRequest('recent-changes.json?' + Math.random(), wf.populateRecentChanges);
    },
    ...
    restoreSettings : function() {
      /* SECTION DEALING WITH BUTTON SETTINGS*/
      ...
      /*SECTION DEALING WITH SORT SETTINGS*/
